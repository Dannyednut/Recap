...
class PolygonContractDeployer:
    """Contract deployment service for Polygon"""
    ...
    def __init__(self, w3: AsyncWeb3, private_key: str):
        ...
        # Gas settings for Polygon (lower than Ethereum/BSC)
        self.gas_price_gwei = 30  # 30 Gwei typical for Polygon
        self.gas_limit_deploy = 2500000  # 2.5M gas for contract deployment
    ...
    async def initialize(self) -> bool:
        ...
        try:
            logger.info("Initializing Polygon Contract Deployer...")
            ...
            logger.info("Polygon Contract Deployer initialized successfully")
            return True
            
        except Exception as e:
            logger.error(f"Failed to initialize Polygon Contract Deployer: {e}")
            return False
    
    async def deploy_arbitrage_contract(self, contract_path: str = None) -> Dict[str, Any]:
        """Deploy arbitrage bot contract"""
        try:
            logger.info(f"Deploying Polygon arbitrage contract")
    ...
    async def deploy_flashloan_contract(self, contract_path: str = None) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Deploying Polygon flash loan contract")
    ...
    async def _deploy_contract(self, name: str, abi: List[Dict], bytecode: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transaction = contract.constructor().build_transaction({
                ...
                'gasPrice': Web3.to_wei(self.gas_price_gwei, 'gwei'),
            ...
            if receipt.status == 1:
                ...
                self.deployed_contracts[name] = {
                    ...
                    "abi": abi,
                    "type": "arbitrage_bot" if name == "ArbitrageBot" else "flashloan",
                    "deployed_at": asyncio.get_event_loop().time()
                }
                
                # Save registry
                await self._save_deployed_contracts()
                
                logger.info(f"Successfully deployed {name} contract at: {receipt.contractAddress}")
    ...
    async def _get_arbitrage_contract_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Simplified arbitrage contract for Polygon
            arbitrage_abi = [
                {
                    "inputs": [],
    ...
    async def estimate_deployment_cost(self, contract_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            matic_price_usd = Decimal("0.8")  # Placeholder
            ...
            return {
                ...
                "cost_matic": float(gas_cost_matic),
